...
def worker(gpu_id, jobs: Queue, lock):
    while True:
        try:
            ...
            if isinstance(params["dist"], tuple):
                dist, lognormal_scale = params["dist"]
            else:
                dist, lognormal_scale = params["dist"], 0.0
            ...
            dist_name = params["dist"] if isinstance(params["dist"], str) else f"{dist}-{lognormal_scale}"
            save_path = os.path.join(
                SAVE_ROOT,
                f"{params['obs_type']}_{params['method']}_seedno{params['seedno']}_"
                f"{params['rand_mask_ratio']}_{params['time_stride']}_"
                f"{dist_name}.pt",
            )
...
if __name__ == "__main__":
    params_to_loop = {
        "obs_type": (["vor2vel"], ["arctan3x"], ["sin3x"]),  # ["vor2vel"], ["arctan3x"], ["sin3x"]
        "method": ("soad",),  # "soad"  # 'soad-full'
        "seedno": (0, 1, 2, 3, 4),  # 1, 2, 3, 4
        "rand_mask_ratio": (0.25,),
        "time_stride": (2,),
        "dist": (
            "normal",
            "laplace",
