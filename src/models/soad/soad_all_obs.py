...
class FreeGeneration(L.Callback):
    ...
    def on_validation_epoch_end(self, trainer: L.Trainer, pl_module: L.LightningModule) -> None:
        ...
        x0 = scheduler.sample(
            x1=torch.randn(1, 5, 10, 256, 256).cuda(),
...
class UNetSDAModule(L.LightningModule):
    def __init__(self, arch: dict, opt: dict, random_crop: bool = True, **kwargs) -> None:
        ...
        self.opHs = [
            get_observation("arctan", scale=3, sigma=0.01),
            get_observation("sinusoidal", freq=3.0, sigma=0.01),
            get_observation("vor2vel", sigma=0.01),
        ]
    ...
    def training_step(self, batch: tuple[torch.Tensor, dict[str, torch.Tensor]], batch_idx: int) -> torch.Tensor:
        ...
        x = torch.cat([x] + [opH.func(x) for opH in self.opHs], dim=-3)
    ...
    hparams = {
        ...
        "opt": {
            ...
            "optimizer": {
                "name": "AdamW",
                "kwargs": {
                    "lr": 1e-4 * bs / 4,  # 2.5e-3 for 1 gpu,  # multiplied by number of gpus
                    "weight_decay": 1e-5 * bs / 4,
                },
            },
            "scheduler": {
                "name": "LambdaLR",
